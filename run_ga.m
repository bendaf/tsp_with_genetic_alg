...
%         tour_size = 5*ones(NIND,1);
%         Chrom = [Chrom tour_size];
        ...
        while gen<MAXGEN
           ...
           % tour_size = Chrom(:,size(Chrom,2));
            %Chrom = Chrom(:,1:size(Chrom,2)-1);
            %visualizeTSP(x,y,adj2path(Chrom(t,:)), minimum, ah1, gen, best, mean_fits, worst, ah2, ObjV, NIND, ah3);
            % if path representation is used, uncomment
        	...
        	SelCh=select('tour', Chrom, ObjV, GGAP);
